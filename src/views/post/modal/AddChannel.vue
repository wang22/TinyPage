<template>
  <div>
    ...
    <t-accordion title="Channel path" mark="channel path" :icon="icons.path" ref="channelPath">
      <template #right>
        <b-form-input placeholder="Enter channel path" v-model="channel.path" @focus="channelNameDetail(true)"></b-form-input>
      </template>
      <template #collapse>
...
export default {
  data () {
    return {
      ...
      switches: {
        memberVisiable: [
          { text: 'Public', value: 'red' },
          { text: 'Member only', value: 'green' },
          { text: 'Paid member only', value: 'yellow' },
          { text: 'Private', value: 'blue' }
        ]
      },
      channel: {
        name: '',
